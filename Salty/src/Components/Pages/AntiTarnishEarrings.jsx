...
const AntiTarnishEarrings = () => {
  ...
  useEffect(() => {
    const fetchAntiTarnishEarrings = async () => {
      try {
        setLoading(true);
        console.log('Fetching Anti-Tarnish Earrings products from: http://localhost:5000/api/products/type/anti-tarnish earrings');
        const response = await axios.get('http://localhost:5000/api/products/type/anti-tarnish earrings');
        console.log('Anti Tarnish Earrings products received:', response.data);
        setProducts(response.data);
        setLoading(false);
      } catch (err) {
        console.error('Error fetching Anti-Tarnish Earrings products:', err);
        setError('Failed to load Anti Tarnish Earrings products. Please try again later.');
        setLoading(false);
      }
    };

    fetchAntiTarnishEarrings();
  }, []);
  ...
  if (loading) return <div className="container mx-auto mt-32 text-center">Loading Anti-Tarnish Earrings products...</div>;
  if (error) return <div className="container mx-auto mt-32 text-center text-red-500">{error}</div>;
  if (products.length === 0) return <div className="container mx-auto mt-32 text-center">No Anti-Tarnish Earrings products found.</div>;
  ...
  return (
    <div className="flex flex-col h-screen">
      ...
      <div className="container mx-auto px-4 mt-10">
        <h1 className="text-3xl font-bold mb-8 text-center mt-30">Anti Tarnish Earrings Collection</h1>
...
export default AntiTarnishEarrings;