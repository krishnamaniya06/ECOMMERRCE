...
const SetOfEarringsProducts = () => {
  ...
  useEffect(() => {
    const fetchSetOfEarringsProducts = async () => {
      try {
        setLoading(true);
        console.log('Fetching Sets of Earrings products from: http://localhost:5000/api/products/type/set of earrings');
        // Fetch only Set of Earrings products
        const response = await axios.get('http://localhost:5000/api/products/type/set of earrings');
        console.log('Sets of Earrings products received:', response.data);
        setProducts(response.data);
        setLoading(false);
      } catch (err) {
        console.error('Error fetching Sets of Earrings products:', err);
        setError('Failed to load Sets of Earrings products. Please try again later.');
        setLoading(false);
      }
    };

    fetchSetOfEarringsProducts();
  }, []);
  ...
  if (loading) return <div className="container mx-auto mt-32 text-center">Loading Sets of Earrings products...</div>;
  if (error) return <div className="container mx-auto mt-32 text-center text-red-500">{error}</div>;
  if (products.length === 0) return <div className="container mx-auto mt-32 text-center">No Sets of Earrings products found.</div>;
    ...
    <div className="flex flex-col h-screen">
      ...
      <div className="flex flex-1 my-35">
        ...
        <div className="container mx-auto px-4 mt-10">
          <h1 className="text-3xl font-bold mb-8 text-center">Set of Earrings Collection</h1>
...
export default SetOfEarringsProducts;